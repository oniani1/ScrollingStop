...
private fun PermissionsStep(context: Context, onNext: () -> Unit) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Text("Permissions", fontSize = 24.sp, fontWeight = FontWeight.Bold, color = Gray100)
        ...
        NextButton("Continue", onNext, enabled = allGranted)
...
private fun LimitsStep(prefs: SecurePreferences, onNext: () -> Unit) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Text("Set Your Limits", fontSize = 24.sp, fontWeight = FontWeight.Bold, color = Gray100)
        Spacer(Modifier.height(24.dp))
        ...
        Slider(
            ...
            colors = SliderDefaults.colors(
                thumbColor = Gray300,
                activeTrackColor = Gray400,
                inactiveTrackColor = Gray800
            )
        )
...
private fun TradingSetupStep(prefs: SecurePreferences, onNext: () -> Unit) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Text("Connect Trading", fontSize = 24.sp, fontWeight = FontWeight.Bold, color = Gray100)
        Spacer(Modifier.height(8.dp))
        Text("Set up at least one (or skip for now)", fontSize = 14.sp, color = Gray600)
        Spacer(Modifier.height(24.dp))

        // Binance section
        Text("Binance API (read-only)", color = Gray400, fontSize = 14.sp, modifier = Modifier.fillMaxWidth())
        Spacer(Modifier.height(8.dp))
        ...
        // Solana section
        Text("Solana Wallet", color = Gray400, fontSize = 14.sp, modifier = Modifier.fillMaxWidth())
        Spacer(Modifier.height(8.dp))
...
private fun ReadyStep(onComplete: () -> Unit) {
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Text("You're All Set", fontSize = 28.sp, fontWeight = FontWeight.Bold, color = Gray100)

        Spacer(Modifier.height(16.dp))

        Text(
            "ScrollingStop will now monitor your screen time " +
        ...
        NextButton("Start Monitoring", onComplete)
    }
}
    ...
    Row(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Icon(
                ...
                tint = if (granted) Gray300 else Gray600,
...
@Composable
private fun NextButton(text: String, onClick: () -> Unit, enabled: Boolean = true) {
    Button(
        onClick = onClick,
        enabled = enabled,
        modifier = Modifier.fillMaxWidth(),
